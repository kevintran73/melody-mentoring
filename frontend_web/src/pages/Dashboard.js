...
const Dashboard = () => {
    ...
    const fetchTrackAttempts = async () => {
      try {
        ...
        let trackDetails = responses.map((response) => {
          const { id, userId, songDetails, isoUploadTime } = response.data;
          return { id, userId, songDetails, isoUploadTime };
        });

        let updatedExperiments = [];
        for (let track of trackDetails) {
          const response = await axios.get(
            `http://localhost:5001/profile/${track.userId}`,
            {
              headers: {
                Authorization: `Bearer ${accessToken}`,
              },
  ...
  const handleClick = (experiment) => {
    navigate(`/review/${experiment.id}`, {
      state: {
        student: experiment.username,
        title: experiment.songDetails.title,
        artist: experiment.songDetails.composer,
      },
    });
  };
  ...
  return (
    <div>
      ...
      <div className='m-10 flex flex-col'>
        ...
        <div className='font-medium text-slate-500 text-lg m-2'>
          When your students request a review, their track attempts will be
            ...
            <div className='flex items-center'>
              <div>
                <span className='font-semibold'>
                  {experiment.songDetails.title}
                </span>{' '}
                <span className='italic'>
                  ({experiment.songDetails.composer})
                </span>{' '}
                - Submitted by {experiment.username}
              </div>
            </div>
            <div
              data-cy={`student-request-${i}`}
